...
const SpaceCard =(props) => {
  ...
  const{mission_name,flight_number,links,mission_id,launch_year,launch_success} = props.data
  return <>
  <div className={styles.spaceContainer}>
    <div className={styles.spaceImageContainer}>
      <img className={styles.imageStyle}src={links?.mission_patch}/>
    </div>
<div className={`${styles.textStyle}`}>{mission_name}<span>#<span></span>{flight_number}</span></div>
    <div>Mission Id:
      {mission_id.length > 0 && mission_id.map(id=>{
        return (
          <>
    ...
    {launch_success?<div className={styles.elementStyle}>True</div>:<div className={styles.elementStyle}>False</div>}
    </div>
    <div className={styles.displayRow}>
